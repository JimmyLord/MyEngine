...
{
    m_matView.CreateLookAt( m_Eye, m_Up, m_At );

    // if the window is taller than we asked for, lock in the horizontal FoV, otherwise lock to the Vertical
    if( m_WindowAspectRatio < m_DesiredGameAspectRatio )
        m_matProj.CreatePerspectiveHFoV( m_DesiredHalfHorizontalFoV, m_WindowAspectRatio, 1, 10000 );
    else
        m_matProj.CreatePerspectiveVFoV( m_DesiredHalfVerticalFoV, m_WindowAspectRatio, 1, 10000 );
...
void Camera3D::SetupProjection(float windowaspectratio, float desiredgameaspectratio, float halfyfov)
{
    ...
    float vertunits = tan( m_DesiredHalfVerticalFoV / 360.0f * PI );
    float horunits = vertunits * m_DesiredGameAspectRatio;
    m_DesiredHalfHorizontalFoV = atan( horunits ) * 360.0f / PI;
